...
def actor(kwargs:dict):
    script = "python async_sac_state_sim.py --actor"
...
def learner(kwargs:dict):
    script = "python async_sac_state_sim.py --learner"
    for k, v in kwargs.items():
        script += f" --{k}"
        script += f" {v}"
    return script
...
def main(_):
    ...
    subprocess.Popen(['tmux', 'new-session', '-s', '-d', 'serl_session'], shell=True)
    ...
    for seed in range(0, 5):
        ...
        actor_process = subprocess.Popen(['tmux', 'send-keys', '-t', 'serl_session:0.0', actor(kwargs=rb_args), 'C-m'], shell=True)


        actor_process.wait()
        subprocess.run("tmux split-window -v", shell=True, check=True)
        rb_args["max_steps"] = max_steps_learner
        learner_process = subprocess.Popen(['tmux', 'send-keys', '-t', 'serl_session:0.1', learner(kwargs=rb_args), 'C-m'], shell=True)
