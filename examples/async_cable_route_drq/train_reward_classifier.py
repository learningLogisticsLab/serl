...
```bash
python examples/async_cable_route_drq/train_reward_classifier.py \
...
def main(_):
    ...
    """
    # Each wrapper changes the interface slightly so the demonstrations and the
    # classifier both see the same observation structure used elsewhere in SERL.
...
def train_reward_classifier(observation_space, action_space):
    ...
    # In this codebase, non-image low-dimensional features usually contain
